...
def get(repository: str, dp: frictionless.package.Package, force: bool = False):
    ...
    for r in range(len(new_dp["resources"])):
        ...
        if new_dp["resources"][r]["format"] == "tif":
            ...
            utilities.download_url(repository+new_dp["resources"][r]["path"],
                                   os.path.basename(new_dp["resources"][r]["path"]))
            raster = {"value" : os.path.basename(new_dp["resources"][r]["path"]),
                      "time":time,
                      "z": None,
                      "unit": unit,
                      "dt": 8760,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 0:
        ...
        parser.add_argument('--select_ds_ids', action='extend', nargs="+", type=int, default=[])
