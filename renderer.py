...
def get_channel_bbox():
    return to_bbox(-6.503906,47.724545,6.679688,52.106505, )
...
def get_bbox():
    ...
    bbox_dim = (6061150.5949,-724011.5319,6819405.9155,743579.4112 )
...
def get_proj4_from_filepath(path):
    raw_srs_info = subprocess.check_output(["gdalsrsinfo", "-o", "proj4", path])
    return raw_srs_info.strip()
...
def render_tile(layer, z, x, y):
        ...
        srs = get_proj4_from_filepath(layer)
        lyr.srs = srs
        ...
        print(lyr.envelope())
        #mp.zoom_all()
        mp.zoom_to_box(get_channel_bbox())
