...
def overlay_light(lau2, shapefile, output):
    ...
    gdf2.geometry = geoms
    join = gpd.sjoin(gdf2, gdf1, how="right").loc[
        :, ["dhPot_2020", "dhPot_2050", "GISCO_ID", "geometry"]
    ]
...
if __name__ == "__main__":
    input_directory = (
        "/media/SSD3.5TB/workspace_mostafa/mostafa_projects/data_warehouse/lau2019"
    )
    output_directory = (
        "/media/SSD3.5TB/workspace_mostafa/mostafa_projects/outputs/res_hc_pathways"
    )
    ...
    Parallel(n_jobs=nr_cores)(
        delayed(overlay_light)(
            country_path_dict[key]["lau2"],
            country_path_dict[key]["shp2"],
            country_path_dict[key]["shp3"],
        )
        for key in country_path_dict.keys()
    )
