...
def rasterstats(geojson, raster_path, factor, stat_types: Optional[List[Text]] = None):
    ...
    ret["graphs"] = {}
    if graph:
        ret["graphs"]["cumulative distribution graph"] = {}
        ret["graphs"]["cumulative distribution graph"]["type"] = "xy"
        ret["graphs"]["cumulative distribution graph"]["values"] = graph
    ret["geofiles"] = {}

    def rounded_statistics(stat: dict) -> dict:
        if not isinstance(stat, dict):
            TypeError("A dictionary is expected.")
        values = dict()
        for key, value in stat.items():
            if value is not None:
                values[key] = round(value, OUTPUT_PRECISION)
        return values

    ret["values"] = rounded_statistics(stat=stat)
